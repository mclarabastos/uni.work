...
if (/^WALLET_MASTER_KEY=\s*$/m.test(env)) {
  ...
  passo('     ela cifra o segredo de cada conta. Não troque sem rotacionar.')
} else {
  ok('chave mestra já configurada')
}

if (!/^HELIUS_API_KEY=.+$/m.test(env)) {
  pendencias.push({
    titulo: 'HELIUS_API_KEY não configurada',
    porque: 'sem ela não há confirmação independente do certificado, e o faucet público costuma recusar',
...
if (bootstrap.status !== 0) {
  pendencias.push({
    titulo: 'o ambiente de devnet não ficou completo',
    porque: 'sem conta com saldo, token de teste e merkle tree, o fluxo on-chain não roda',
    como: 'siga a instrução impressa acima e rode npm run setup de novo (ele retoma de onde parou)'
  })
}
...
if (!temAnchor || !temCargo) {
  aviso('toolchain Anchor não encontrado, pulando a compilação e o deploy')
  passo('     o driver vault funciona sem deploy nenhum, então o produto roda assim mesmo')
  pendencias.push({
    titulo: 'programa Anchor não deployado',
  ...
  if (fs.existsSync(copiaDoKeypair) && !fs.existsSync(keypairDoPrograma)) {
    ...
    ok('endereço do programa restaurado de .uniwork/: continua sendo o mesmo de antes')
  }

  passo('sincronizando o endereço declarado no programa…')
  ...
  if (sync.status !== 0) {
    ...
    aviso('anchor keys sync não rodou; seguindo com o driver vault')
    pendencias.push({
      titulo: 'declare_id! não sincronizado com o endereço do programa',
      porque: 'com os dois diferentes o programa deploya e depois recusa toda instrução, o que parece deploy bem-sucedido',
      como: 'rode anchor keys sync na mao e veja o erro; até la o driver vault continua funcionando'
    })
  } else {
    process.stdout.write(sync.stdout ?? '')
    ok('declare_id! e Anchor.toml apontam para o endereço real do programa')
    ...
    if (build.status !== 0) {
      pendencias.push({
        ...
        porque: 'sem o programa compilado não há deploy',
      ...
      if (deploy.status === 0 && encontrado) {
        ...
        aviso('o deploy não concluiu; seguindo com o driver vault')
        pendencias.push({
...
if (migrou.status !== 0) {
  console.error('\n  as migrations falharam. Sem banco o resto não anda.\n')
  process.exit(1)
}

const semeou = rodarNode('seed.js')
if (semeou.status !== 0) {
  aviso('o seed não completou; o banco está migrado, só sem dado de exemplo')
}

// ─── 6. diagnostico ──────────────────────────────────────────────────────────
cabecalho('6. diagnóstico')
rodarNode('doctor.js')

// ─── fecho ───────────────────────────────────────────────────────────────────
if (pendencias.length) {
  console.log(`  ${pendencias.length} pendência(s) que precisam de você:\n`)
...
console.log(`  para subir o serviço:
